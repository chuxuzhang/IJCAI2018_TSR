...
if train_test_label == 0:
	...
	p_pos_index = np.arange(0, batch_s * 2, 2)
	p_neg_index = np.arange(1, batch_s * 2, 2)

	a_e = tf.gather(author_embed, a_p_p_dir[:,0])
	a_e = tf.reshape(a_e, [batch_s, embed_d])
	p_e_pos = tf.gather(p_c_dir_e, p_pos_index)
	p_e_pos = tf.reshape(p_e_pos, [batch_s, embed_d])
	p_e_neg = tf.gather(p_c_dir_e, p_neg_index)
	p_e_neg = tf.reshape(p_e_neg, [batch_s, embed_d])

	pos_score = tf.reduce_sum(tf.multiply(a_e, p_e_pos), 1, keep_dims = True)
	neg_score = tf.reduce_sum(tf.multiply(a_e, p_e_neg), 1, keep_dims = True)

	# pairwise ranking loss
	#diff = tf.matmul(a_e, p_e_pos)) - tf.matmul(a_e, p_e_neg)
	diff = tf.subtract(pos_score, neg_score)
	Loss_1 = - tf.log(tf.sigmoid(diff))
	#Loss_1.append(-tf.log(tf.sigmoid(diff)))

	# loss of indirect relation
	#Loss_2 = []
	#for j in range(batch_s):
	a_e = tf.gather(author_embed, a_p_p_indir[:, 0])
	a_e = tf.reshape(a_e, [batch_s, embed_d])
	p_e_pos = tf.gather(p_c_indir_e, p_pos_index)
	p_e_pos = tf.reshape(p_e_pos, [batch_s, embed_d])
	p_e_neg = tf.gather(p_c_indir_e, p_neg_index)
	p_e_neg = tf.reshape(p_e_neg, [batch_s, embed_d])

	pos_score = tf.reduce_sum(tf.multiply(a_e, p_e_pos), 1, keep_dims = True)
	neg_score = tf.reduce_sum(tf.multiply(a_e, p_e_neg), 1, keep_dims = True)

	margin_n_p = tf.subtract(neg_score, pos_score) + tf.constant(margin_d, shape=[batch_s, 1])
	constant_zero = tf.zeros([batch_s, 1])
	margin_loss = tf.concat([margin_n_p, constant_zero], 1)
	# distance margin loss
	#hinge_l = tf.maximum(margin_d + margin_n_p, tf.zeros([batch_s, 1]))
	hinge_l = tf.reduce_max(margin_loss, 1)
	Loss_2 = hinge_l
...
# TSR model (objective function formulation) end #
